...
public interface CalendarEventRepository extends JpaRepository<CalendarEvent, Long> {
	...
	@Query("SELECT e FROM CalendarEvent e WHERE e.paciente.userId = :userId AND "
			+ "(LOWER(e.title) LIKE LOWER(:searchTerm) OR LOWER(e.description) LIKE LOWER(:searchTerm) OR LOWER(e.paciente.name) LIKE LOWER(:searchTerm))")
	List<CalendarEvent> findByUserIdAndSearchTerm(@Param("userId") String userId, @Param("searchTerm") String searchTerm);
