...
public class PlatilloServiceImpl implements PlatilloService {
  ...
  public Ingrediente addIngrediente(@NonNull Long id, @NonNull Long alimentoId, @NonNull String cantidad,
    ...
    if (!cantidad.equals(ingrediente.getAlimento().getFractionalCantSugerida())) {
      ...
      calculateIngredienteFromCantiadChange(cantidad,
          alimento.getFractionalCantSugerida(), ingrediente, alimento);
  ...
  private void addMacronutrientesPlatillo(Platillo platillo, Ingrediente ingrediente) {
    ...
    if (ingrediente.getAzucarPorEquivalente() != null)
      platillo.setAzucarPorEquivalente(platillo.getAzucarPorEquivalente() + ingrediente.getAzucarPorEquivalente());
