...
public final class WebContextFactory {
	...
	private static IWebRequest createMockWebRequest(final HttpServletRequest request) {
		return new MockWebRequestImpl(request);
	}

	private static final class MockWebRequestImpl implements IWebRequest {

		private final HttpServletRequest request;

		MockWebRequestImpl(final HttpServletRequest request) {
		...
		public String[] getHeaderValues(final String name) {
			return extractHeaderValues(request, name);
		}

		@Override
		public Set<String> getAllHeaderNames() {
			return extractAllHeaderNames(request);
		}
		...
		public String getCookieValue(final String name) {
			return extractCookieValue(request, name);
		}

		@Override
		public Set<String> getAllCookieNames() {
			return extractAllCookieNames(request);
		}

		@Override
		public boolean containsCookie(final String name) {
			return checkCookieExists(request, name);
		}
		...
		public int getCookieCount() {
			return countCookies(request);
		}

		@Override
		public java.util.Map<String, String[]> getCookieMap() {
			return buildCookieMap(request);
		}
		...
		public String[] getCookieValues(final String name) {
			return extractCookieValues(request, name);
		}

	}

	private static String[] extractHeaderValues(final HttpServletRequest request, final String name) {
	...
	private static Set<String> extractAllHeaderNames(final HttpServletRequest request) {
	...
	private static String extractCookieValue(final HttpServletRequest request, final String name) {
	...
	private static Set<String> extractAllCookieNames(final HttpServletRequest request) {
		final jakarta.servlet.http.Cookie[] cookies = request.getCookies();
		final Set<String> result = new java.util.HashSet<>();
		if (cookies != null) {
			for (final jakarta.servlet.http.Cookie cookie : cookies) {
	...
	private static boolean checkCookieExists(final HttpServletRequest request, final String name) {
		final jakarta.servlet.http.Cookie[] cookies = request.getCookies();
		if (cookies != null) {
			for (final jakarta.servlet.http.Cookie cookie : cookies) {
				if (name.equals(cookie.getName())) {
	...
	private static int countHeaders(final HttpServletRequest request) {
		final Enumeration<String> headerNames = request.getHeaderNames();
		int count = 0;
		if (headerNames != null) {
			while (headerNames.hasMoreElements()) {
	...
	private static java.util.Map<String, String[]> buildHeaderMap(final HttpServletRequest request) {
	...
	private static int countCookies(final HttpServletRequest request) {
		final jakarta.servlet.http.Cookie[] cookies = request.getCookies();
		return cookies != null ? cookies.length : 0;
	}

	private static java.util.Map<String, String[]> buildCookieMap(final HttpServletRequest request) {
		final java.util.Map<String, String[]> map = new HashMap<>();
		final jakarta.servlet.http.Cookie[] cookies = request.getCookies();
		if (cookies != null) {
			for (final jakarta.servlet.http.Cookie cookie : cookies) {
	...
	private static String[] extractCookieValues(final HttpServletRequest request, final String name) {
	...
	private static final class MockServletContextImpl implements ServletContext {
	...
	private static HttpServletRequest createMockHttpServletRequest(final ServletContext servletContext) {
		return new MockHttpServletRequestImpl(servletContext);
	}

	private static final class MockHttpServletRequestImpl implements HttpServletRequest {

		private final Map<String, Object> attributes = new HashMap<>();

		private final ServletContext servletContext;

		MockHttpServletRequestImpl(final ServletContext servletContext) {
	...
	private static final class MockHttpServletResponseImpl implements HttpServletResponse {
