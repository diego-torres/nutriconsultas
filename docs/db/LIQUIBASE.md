...
| `changes/002-seed-data.yaml` | Catalog seed: `alimento`, `seed_platillo` auxiliary tables |
| `data/alimentos-seed.sql` | SMAE alimentos catalog (from `alimentos.sql`) |
| `data/seed-platillos-aux.sql` | Platillo auxiliary rows (from `seed_platillos.sql`) |

**Startup seeders (Java):** `PlatilloSeedInitializer` materializes `platillo`/`ingrediente` entities from auxiliary tables; `DietaTemplateSeedInitializer` builds template dietas. These run after Liquibase on empty databases.
...
3. Seed changesets run only when `alimento` / `seed_platillo` are empty.
