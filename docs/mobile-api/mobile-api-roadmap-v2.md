...
| **96** | `GET /rest/mobile/patient/messages` | Message thread | Yes | Cursor pagination (not offset); no message body in INFO logs. Optional `senderDisplayName` from `NutritionistProfile.displayName` when `senderRole=NUTRITIONIST` (#116, done). |
