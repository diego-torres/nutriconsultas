...
>
> - **Backend schema (2026-06-12):** Phase 0 **#107 implemented** in [PR #117](https://github.com/diego-torres/nutriconsultas/pull/117) — `patientAuthSub`, `oauth2-resource-server`, `MobileSecurityConfig`, `PatientAuthService`, linkage filter. **#110 still open** (`ApiResponse`/`PagedResponse`). No `/rest/mobile/patient/**` feature controllers yet; #91–#99 remain blocked until #110 lands. No message entity (#96/#97 greenfield). `NutritionistProfile.displayName` at 228bbc3; #116 for optional `senderDisplayName`.
...
### G5 — Messaging greenfield (#96, #97) (unchanged)
